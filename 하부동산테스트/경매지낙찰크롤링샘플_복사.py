...
def clean_table_data(aution_item):
    ...
    aution_item = aution_item[['날짜', '법원', '사건번호', '물건번호', '용도', '감정평가액',
                               '최저가격', '최저비율', '유찰횟수', '소재지', '내역', '비고']]
...
if __name__ == "__main__":
    print("=== 법원 경매 데이터 크롤러 ===")
    print("지원(예: 서울중앙지방법원, 서울동부지방법원, 서울서부지방법원):")
    jiwon = input("지원: ").strip()
    print("건물 유형(예: 단독주택, 다가구주택, 다중주택, 아파트, 연립주택, 다세대주택, 기숙사, 빌라, 상가주택, 오피스텔, 주상복합):")
    building = input("건물 유형: ").strip()
