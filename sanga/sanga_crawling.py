...
def main():
    ...
    for umdNm, cortarNo in areas.items():
        for page in range(1, totPage):  # 페이지 1~99
            # SG:SMS:GM : 상가,사무실,건물
            #url = f'https://m.land.naver.com/api/articles?cortarNo={cortarNo}&order=prcDesc&realEstateType=SG:SMS:GM%3ASMS&tradeType=월세&page={page}'
            url = f'https://m.land.naver.com/cluster/clusterList?view=atcl&cortarNo={cortarNo}&rletTpCd=SG&tradTpCd=A1'
            headers = get_random_headers()
            response = session.get(url, headers=headers)
            print(response.text)
            try:
