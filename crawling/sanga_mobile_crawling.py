...
from crawling.sanga_mobile_db_utils import sanga_save_to_sqlite
#
# WAL 모드 설정 => sqlite3 연결 시 PRAGMA journal_mode=WAL 설정 필요
from crawling.sanga_mobile_db_utils import sqlite_enable_wal, install_sqlite_shutdown_hooks, _safe_checkpoint
...
def normalize_keyword_old(keyword: str) -> str:
    """
    '김포시 운양동' 같이 공백 포함 검색어를 받으면
    - 동(읍/면/리) 단어가 있으면 그걸 우선 사용
    - 없으면 원문 그대로
    """
    kw = keyword.strip()
    # 동/읍/면/리 후보 추출
    tokens = kw.split()
    dong_like = [t for t in tokens if t.endswith(("동","읍","면","리"))]
    if dong_like:
        return dong_like[-1]  # 가장 오른쪽 항목 우선
    return kw
...
def savd_db_dict(session: requests.Session, items: List[ArticleItem], page: int, keyword: str):
    """
    ArticleItem 리스트(items)를 받아 DB 저장용 dict 리스트를 구성 후 sanga_save_to_sqlite()로 저장
    """
    if not items:
        print("저장할 데이터가 없습니다.")
        return
...
def make_keywords_from_search(name_like: str = "", trade_type: str="SG") -> Dict[str, str]:
    """
...
if __name__ == "__main__":
    ...
    # 1) 시작 시 한 번: WAL 활성화 + 동기화 레벨 설정
    sqlite_enable_wal(synchronous="NORMAL")  # or "FULL" / "OFF"

    # 2) 종료 훅 설치(정상/예외/신호 종료 시 체크포인트 수행)
    install_sqlite_shutdown_hooks(mode="FULL")  # 또는 "TRUNCATE"

    try:
        # (예) 여러 지역을 한 번에 처리 (원하시는 만큼 추가)
        # keywords = {
        #     "경기도 김포시 운양동": "4157010300",
        # }
        # (예) "운양동"이 포함된 데이터만 가져와 keywords 생성
        keywords = make_keywords_from_search(name_like="운양동", trade_type="SG")
        ...
        # 3) 종료 시 한 번: WAL 비활성화
        _safe_checkpoint("FULL")   # 또는 "TRUNCATE"
