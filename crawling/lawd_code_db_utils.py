...
def init_lawd_db(db_path: str = DB_PATH) -> None:
    """
    lawdcd_data(lawd_cd TEXT, lawd_name TEXT)
...
# ==========================
# 4) lawd_name으로 다건 검색 (부분 일치)
# ==========================
def search_lawd_by_name(name_keyword: str, db_path: str = DB_PATH, limit: int = 30) -> List[Dict[str, str]]:
    """
    법정동명으로 부분 검색 (LIKE '%키워드%')
    ...
    try:
        ...
        LIMIT ?
        """
        cur = conn.execute(sql, (f"%{name_keyword}%", limit))
        rows = cur.fetchall()
        return [{"lawd_cd": r[0], "lawd_name": r[1]} for r in rows]
    finally:
        conn.close()
...
if __name__ == "__main__":
   ...
   # 예시: "운양동"으로 검색
    results = search_lawd_by_name("운양동")
    for r in results:
        print(r)