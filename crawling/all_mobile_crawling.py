...
from crawling.apt_mobile_db_utils import apt_save_to_sqlite, apt_delete_by_lawdCd_umdNm
from crawling.sanga_mobile_db_utils import sanga_save_to_sqlite, sanga_delete_by_lawdCd_umdNm
...
def iter_group_pages(session: requests.Session, group: Dict[str, Any],
    ...
    for page in range(1, max_page + 1):
        ...
        time.sleep(random.uniform(45, 65))  # 10~20초 랜덤 대기 (차단 회피)
        #
...
# -----------------------------
def parse_articles(session: requests.Session, payload: Dict[str, Any], keyword: str, lawdCd: str, ttype: str) -> List[ArticleItem]:
    ...
    - 대응 형태:
      ...
      D) [ {...}, ... ]  # 루트가 리스트인 경우
    """
    print("parse_articles 호출됨...=> " + ttype)
    ...
    # DB 저장 예시 (필요 시)
    savd_db_dict(session, items, page=1, keyword=keyword, ttype=ttype)
...
# -----------------------------
# DB 저장 예시
def savd_db_dict(session: requests.Session, items: List[ArticleItem], page: int, keyword: str, ttype: str):
    """
    ...
    # === DB 저장 ===
    try:
        if (ttype or "").upper() == "APT":
            apt_save_to_sqlite(data_entries)
        else:
            sanga_save_to_sqlite(data_entries)
        print(f"DB 저장 완료: {len(data_entries)}건 (page={page}, {keyword}, ttype={ttype})")
    except Exception as e:
...
def scrape_one_keyword(session: requests.Session,
    ...
    for g in groups:
        for payload in iter_group_pages(session, g, rlet, trad, filt.cortarNo, verify=verify):
            items = parse_articles(session, payload, normalize_keyword(keyword), lawdCd, ttype)
...
def scrape_keywords(keywords: Dict[str, str],
    ...
    for kw, meta in keywords.items():
        lawdCd = meta["lawd_cd"]
        ttype = (meta.get("trade_type") or "APT").upper()
        print(f"\n▶ 키워드 처리 시작: {kw} (ttype={ttype})")
        try:

            # 1) 실제 검색용 키워드는 괄호 제거하여 전달
            search_kw = (kw
                .replace(" (APT)", "")
                .replace("(APT)", "")
                .replace(" (SG)", "")
            ...
            # 🔹 1-1) 읍면동명(umdNm) 자동 추출
            # 예: "충청북도 청주시 서원구 개신동" → "개신동"
            parts = search_kw.split()
            umdNm = parts[-1] if parts else ""
            print(f"=== search_kw => {search_kw}, umdNm => {umdNm}")

            # 2) 타입별 기존 데이터 삭제 후 0.5초 대기
            if ttype == "APT":
                deleted = apt_delete_by_lawdCd_umdNm(lawdCd[:5], umdNm)
            else:
                deleted = sanga_delete_by_lawdCd_umdNm(lawdCd[:5], umdNm)
            if verify:
                dprint(f"[정리] lawdCd={lawdCd[:5]} umdNm='{umdNm}' (type={ttype}) 기존 {deleted}건 삭제")
            time.sleep(5)

            # 2) 크롤링 시작
            rlet_local = "APT" if ttype == "APT" else "SG"
            items = scrape_one_keyword(session, search_kw, lawdCd, rlet_local, tradTpCds, lat_margin, lon_margin, ttype=ttype, verify=verify)
            print(f"▶ 키워드 처리 완료: {kw} (수집 {len(items)}건, type={ttype})")
...
def make_keywords_from_search(name_like: str = "", trade_type: str="") -> Dict[str, str]:
    """
    반환 예:
      {
        "경기도 김포시 운양동 (APT)": {"lawd_cd": "4157010300", "trade_type": "APT"},
        "경기도 김포시 운양동 (SG)" : {"lawd_cd": "4157010300", "trade_type": "SG"},
      }
    ※ 키에는 타입을 붙여 중복을 피하고, 실제 검색어는 normalize_keyword()로 타입 표기를 제거해 사용.
    """
    ...
    # 같은 lawd_name에 대해 APT/SG 등 다건이 올 수 있으므로, 타입별로 각각 키를 만든다.
    keywords: Dict[str, dict] = {}
    for row in rows:
        full_name = row["lawd_name"].strip()           # 예: "경기도 김포시 운양동"
        row_type  = (row.get("trade_type") or trade_type or "APT").upper()  # 우선순위: row > 인자 > 기본
        key_name  = f"{full_name} ({row_type})"        # dict 키는 중복 방지용으로 타입을 붙인다.
        keywords[key_name] = {
            "lawd_cd": row["lawd_cd"],
            "trade_type": row_type
        }
    ...
    for k, v in keywords.items():
        print(f'"{k}": {{"lawd_cd": "{v["lawd_cd"]}", "trade_type": "{v["trade_type"]}"}},')
...
if __name__ == "__main__":
    ...
    keywords = make_keywords_from_search(name_like="", trade_type="")
