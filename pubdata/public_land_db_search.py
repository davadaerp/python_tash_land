...
def fetch_land_month(url: str, params: dict, lawd_nm: str, umd_nm: str, year: int, month: int, verify: bool = False):
    ...
    for year in years:
        ...
        if month_items:
            ...
            for month in range(1, 13):
                ...
                month_items = fetch_land_month(url, params, lawd_nm, umd_nm, year, month, verify=verify)
                # DB저장
                if month_items:
...
def run_villa(lawd_cd: str, lawd_nm:str, umd_nm: str,  verify: bool = False):
    ...
    for year in years:
        ...
        year_items = []
        # 1) DB 먼저 조회-(읍면동, 년도비교로 존재여부 체크)
        month_items = read_villa_db(lawd_cd, umd_nm, year)
        if month_items:
            print(f"{year}년 : DB에서 {len(month_items)}건 조회됨")
            #
            year_items.extend(month_items)
