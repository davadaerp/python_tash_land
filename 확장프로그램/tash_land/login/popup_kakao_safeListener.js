...
console.count("popup_kakao.js loaded");

// ✅ DOM 엘리먼트는 "로딩 후"에 잡는다
let loginSection, userSection, nicknameEl, smsCountEl, subStatusEl, subscribeWrap;
let btnCharge, btnLogin, btnLogout, btnMypage, toast;

function cacheEls() {
  loginSection  = document.getElementById("login-section");
  userSection   = document.getElementById("user-section");
  nicknameEl    = document.getElementById("nickname");
  smsCountEl    = document.getElementById("sms-count");
  subStatusEl   = document.getElementById("sub-status");
  subscribeWrap = document.getElementById("subscribe-wrap");
  btnCharge     = document.getElementById("btn-charge");
  btnLogin      = document.getElementById("btn-login");
  btnLogout     = document.getElementById("btn-logout");
  btnMypage     = document.getElementById("btn-mypage");
...
// 안전한 이벤트 리스너 추가 함수
function safeAddListener(el, event, handler, name = "unknown", options) {
  if (!el) {
    console.warn(`⚠️ addEventListener 대상이 없습니다: ${name}`);
    return false;
  }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("btnMypage", btnMypage);
    console.log("btnCharge", btnCharge);

    // 안전 로그인처리
    safeAddListener(btnLogin, "click", loginWithKakao, "btn-login");
    // 안전 로그아웃처리
    safeAddListener(btnLogout, "click", logoutWithKakao, "btn-logout");
    // 안전 마이페이지
    safeAddListener(btnMypage, "click", myPage, "btn-mypage");
    // 안전 문자충전
    safeAddListener(btnCharge, "click", smsCharge, "btn-charge");

    restoreSession();          // ✅ 캐싱 후 호출
});
