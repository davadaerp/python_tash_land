...
function summaryAptVillaAndDisplayTable() {
		...
		// 값산출
		const tableDanga1 = tableData1.map(item => item.가격);
		console.log('필터된 데이터', tableDanga1);
		if (tableDanga1.length > 0) {
			let minPdanga = Math.min(...tableDanga1);
			let maxPdanga = Math.max(...tableDanga1);
			let avgPdanga = tableDanga1.reduce((sum, value) => sum + value, 0) / tableDanga1.length;
			...
			tableDataSum.push(['월세', minPdanga.toFixed(1), avgPdanga.toFixed(1), maxPdanga.toFixed(1), tableDanga1.length]);
		}
...
function realSearchData() {
	...
	realDataSearch.id = 'realDataSearch'; // 1. id 지정
	realDataSearch.textContent = '실거래검색';
	realDataSearch.style.marginLeft = '10px'; // modeText와의 간격 조절
	realDataSearch.style.backgroundColor = '#007bff'; // 파란색 배경
	realDataSearch.style.color = '#fff'; // 흰색 텍스트
	realDataSearch.style.padding = '4px 8px'; // 내부 여백 줄이기
	realDataSearch.style.fontSize = '12px'; // 작은 글씨 크기
	realDataSearch.style.borderRadius = '8px'; // 둥근 모서리
	realDataSearch.style.cursor = 'pointer'; // 클릭 가능 커서
	realDataSearch.style.transition = 'all 0.3s ease'; // 부드러운 애니메이션 효과

	// 특정 div 생성 (모달)
	const modalDiv = document.createElement('div');
	modalDiv.id = "customPopup";
	modalDiv.innerHTML = `<div id="popupHeader">드래그하여 이동</div><p>실거래검색 팝업</p>`;
	//
	modalDiv.style.width = '900px';
	modalDiv.style.height = '1030px';
	modalDiv.style.backgroundColor = '#f0f0f0';
	modalDiv.style.border = '1px solid #ddd';
	modalDiv.style.position = 'absolute'; // 고정된 위치
	modalDiv.style.display = 'none'; // 기본적으로 숨김 처리
	modalDiv.style.opacity = '0'; // 초기 opacity 0 (숨김)
	modalDiv.style.transition = 'opacity 0.5s ease'; // 부드러운 opacity 애니메이션
	...
	closeButton.style.color = 'black';            // 텍스트 색 검정색
	closeButton.style.border = '1px solid black';   // 네모난 테두리 추가
	closeButton.style.borderRadius = '0';           // 둥근 모서리 제거

	// 닫기 버튼 클릭 시 모달 닫기 기능
	closeButton.addEventListener('click', function() {
	  modalDiv.style.opacity = '0'; // 부드럽게 사라지게 opacity 0으로 변경
	  setTimeout(() => {
		modalDiv.style.display = 'none'; // 애니메이션 후 모달 숨김
	  }, 500);
	});
	// 모달에 닫기 버튼 추가
	...
	realDataSearch.addEventListener('click', function() {
	  modalDiv.style.display = 'block'; // 모달 표시
	  setTimeout(() => {
		modalDiv.style.opacity = '1'; // 부드럽게 나타나게 opacity 1로 변경
	  }, 5);

	  // 2. 지정된 span(#realDataSearch)의 왼쪽 아래 모서리를 기준으로 모달 위치 지정
	  const spanRect = realDataSearch.getBoundingClientRect();
	  modalDiv.style.top = (spanRect.bottom - 100) + 'px';
	  modalDiv.style.left = spanRect.left + 'px';

	  // 확장 프로그램의 HTML 파일 경로 가져오기
	  const externalHtmlPath = chrome.runtime.getURL('main.html');
	  // iframe을 사용하여 HTML 파일 로드
	  iframe.src = externalHtmlPath;
	});

	// realDataSearch 요소에 모달(div) 추가
	realDataSearch.appendChild(modalDiv);
	...
	header.addEventListener("mousedown", (e) => {
	  isDragging = true;
	  offsetX = e.clientX - modalDiv.getBoundingClientRect().left;
	  offsetY = e.clientY - modalDiv.getBoundingClientRect().top;
	});
...
function extractDepositAndRent(text) {
    ...
    if (match) {
        let deposit = parseInt(match[1].replace(/,/g, ""), 10) * 10000;
	...
	if (onoffstatus == true) {
		...
		if (propertyItems != oldItems && propertyItems.length > 0) {
			...
			propertyItems.forEach((item) => {
				...
				/*
				// 1. 숫자와 슬래시(/)만 남기고 나머지 문자 삭제
				const cleanedSecondPart = secondPart ? secondPart.replace(/[^0-9고중저\/]/g, '') : '';
				//console.log('villa cleanedSecondPart = ' + cleanedSecondPart);

				// 2. 슬래시가 있을 때, 슬래시 뒤 값이 없으면 처리
				let finalSecondPart = cleanedSecondPart;

				// 슬래시가 포함되어 있다면
				if (finalSecondPart.includes('/')) {
				  const parts = finalSecondPart.split('/'); // parts[0]: 슬래시 앞 값, parts[1]: 슬래시 뒤 값

				  // 1. parts[0]가 '고', '중', '저'인 경우 처리
				  if (parts[0] === '고' || parts[0] === '중' || parts[0] === '저') {
					if (parts[1]) {
					  // parts[1]에 '층'이 포함되어 있을 수 있으므로 제거하고 숫자로 변환
					  const totalFloor = Number(parts[1].replace(/층/g, ''));
					  if (!isNaN(totalFloor)) {
						if (parts[0] === '고') {
						  // '고'이면 전체층(totalFloor)을 할당
						  parts[0] = totalFloor.toString();
						} else if (parts[0] === '중') {
						  // '중'이면 전체층의 중간값 (소수점 버림)
						  parts[0] = Math.floor(totalFloor / 2).toString();
						} else if (parts[0] === '저') {
						  // '저'이면 최저층인 1을 할당
						  parts[0] = '1';
						}
				  ...
				  // 2. 슬래시 뒤 값이 없는 경우 처리
				  if (!parts[1] || parts[1] === '') {
					// parts[0]에 '층'이 포함되어 있을 수 있으므로 제거하고 숫자로 변환
					const numBefore = Number(parts[0].replace(/층/g, ''));
					parts[1] = isNaN(numBefore) ? '0' : parts[0];
				  }
				  finalSecondPart = parts.join('/');
				}
				console.log('finalSecondPart:', finalSecondPart);
				*/
				const floorMatch = secondPart ? secondPart.match(/(\d+)\/(\d+)층/) : null;
				const cfloor = floorMatch ? floorMatch [1] : null; // 매물층
				const tfloor = floorMatch ? floorMatch [2] : null; // 전체층
				...
				if (areas > 0) {
					if (type == '월세') {
						...
						// 테이블에 넣기
						const tableitem = { 구분: type, 해당층: cfloor, 전체층: tfloor, 향: direction, 평당가: pdanga, 전용면적: areas3, 가격: salePrice };
		...
		if (mapWrap) {
			...
			if (propertyData.length > datalength1 || propertyData2.length > datalength2 || propertyData3.length > datalength3) {
				...
				// 월세 억단위변환
				const maxValue_Eok = (!maxValue || !isFinite(maxValue)) ? "0억" : formatToEok(maxValue) + "억";
				const minValue_Eok = (!minValue || !isFinite(minValue)) ? "0억" : formatToEok(minValue) + "억";
				const avgValue_Eok = (!avgValue || !isFinite(avgValue)) ? "0억" : formatToEok(avgValue) + "억";
