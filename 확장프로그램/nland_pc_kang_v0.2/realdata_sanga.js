    ...
    function updateSortPyeongIcon() {
      const sortPyeongEl = document.getElementById("sortPyeong");
      if (sortPyeongAscending) {
        sortPyeongEl.innerHTML = "건물면적(평) ▲";
      } else {
        sortPyeongEl.innerHTML = "건물면적(평) ▼";
      }
    }

    function updateSortDealDateIcon() {
      const sortDealDateEl = document.getElementById("sortDealDate");
      if (sortDealDateAscending) {
        sortDealDateEl.innerHTML = "거래일자 ▲";
      } else {
        sortDealDateEl.innerHTML = "거래일자 ▼";
      }
    }

    function updateSortFloorIcon() {
      const sortFloorEl = document.getElementById("sortFloor");
      if (sortFloorAscending) {
        sortFloorEl.innerHTML = "층 ▲";
      } else {
        sortFloorEl.innerHTML = "층 ▼";
      }
    }
      ...
      let tableHTML = `<table>
        ${items.map((item, index) => {
         ...
         // 각 셀을 index에 따라 정렬 처리: 4번째 셀은 오른쪽, 나머지는 중앙 정렬
          ...
          const mapBtnHTML = `<td><button id="map-btn-${currentIndex}" style="padding:2px 4px; font-size:12px;">지도</button></td>`;
          const detailBtnHTML = `<td><button id="detail-btn-${currentIndex}" style="padding:2px 4px; font-size:12px;">상세</button></td>`;
