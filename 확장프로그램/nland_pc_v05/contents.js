...
function extractPropertyInfo3() {
	// Find the div with id "auct_list"
    let auctionList = document.getElementById("auct_list");
    if (!auctionList) return;

    // Find the table with class "tbl_list" inside "auct_list"
    let table = auctionList.querySelector(".tbl_list");
    if (!table) return;

    // Find the second tbody
    let tbodies = table.getElementsByTagName("tbody");
    if (tbodies.length < 2) return;
    let secondTbody = tbodies[1];

    // Get all <tr> elements inside the second tbody
    const propertyItems = secondTbody.querySelectorAll('tr');
    ...
    propertyItems.forEach((item) => {
		...
		//==================================================================
		// 건물 or 토지 평형구하기
		let { landArea, buildingArea } = getAreaPy(targetTd4);
		console.log("Land Area:", landArea, "평");
		console.log("Building Area:", buildingArea, "평");

		if (landArea === 0 && buildingArea === 0) {
			return;
		}
		let areaPy = buildingArea === 0 ? landArea : buildingArea;

		//==================================================================
		// 감정가/최거매각가 금액을 가져온다.
		let priceData = getPrices(targetTd5);
		 // priceData.forEach(item => {
			//  console.log(`${item.id}: ${item.value}`);
		 // });
		 //==================================================================
		 // 금액1 추출 (auct_iprice_숫자 아이디 내부 텍스트) //auct_jprice_2406369
		const price1Element = document.getElementById(priceData[0].id);
		const price1num = parseInt(priceData[0].value.replace(/,/g, ''));
		console.log(price1num);

		// 금액2 추출 (auct_iprice_숫자 아이디 내부 텍스트) //auct_jprice_2406369
		const price2Element = document.getElementById(priceData[1].id);
		const price2num = parseInt(priceData[1].value.replace(/,/g, ''));
		console.log(price2num);

		const pydanga1 = parseInt(price1num / (areaPy * 10000));
		const pydanga2 = parseInt(price2num / (areaPy * 10000));

		console.log(pydanga1, pydanga2);
...
function getAreaPy(targetTd4) {
	let areaText;
	// Search for elements in targetTd4 containing "건물" or "평"
	let allElements = targetTd4.querySelectorAll("*");
	for (let element of allElements) {
		let text = element.textContent.trim();
		if (text.includes("건물") || text.includes("평")) {
			//console.log("4th TD contains:", text);
			areaText = text;
			break;
		}
	}
	console.log("4th TD contains:", areaText);
	if (!areaText) {
        //console.error("getAreaPy: tdElement is null, undefined, or has no text content");
        return { landArea: 0, buildingArea: 0 };
    }
    // Extract the full substring for land and building
    let landSubstring = areaText.match(/토지.*?평/);
    let buildingSubstring = areaText.match(/건물.*?평/);

    let landArea = 0;
    let buildingArea = 0;

    if (landSubstring) {
        let landMatch = landSubstring[0].match(/(\d+\.\d*)평/);
        landArea = landMatch ? parseFloat(landMatch[1]) || 0 : 0;
    }

    if (buildingSubstring) {
        let buildingMatch = buildingSubstring[0].match(/(\d+\.\d*)평/);
        buildingArea = buildingMatch ? parseFloat(buildingMatch[1]) || 0 : 0;
    }
...
// 감정가/최거매각가 금액을 가져온다.
function getPrices(targetTd5) {
	// Get div elements inside the target 5th <td>
	let divs5 = targetTd5.getElementsByTagName("div");

	// Extract ID and values from 5th td
	let result5 = [];
	for (let div of divs5) {
		if (div.id) {
			result5.push({ id: div.id, value: div.textContent.trim() });
		}
	}
	console.log("5th TD Divs:", result5);
	return result5;
}


function extractPropertyInfo3_old() {
